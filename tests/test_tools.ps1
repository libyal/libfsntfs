...
Foreach (${TestName} in ${ToolsTests} -split " ")
{
	...
	$Result = RunTestBinary ${TestExecutablesDirectory} "fsntfs_test_tools_${TestName}"
...
$Profiles = @("fsntfsinfo", "fsntfsinfo_fs", "fsntfsinfo_mft", "fsntfsinfo_usn", "fsntfsinfo_bodyfile_fs", "fsntfsinfo_bodyfile_mft")
...
For ($ProfileIndex = 0; $ProfileIndex -le ($Profiles.length - 1); $ProfileIndex += 1)
{
	...
	{
		$Result = RunToolsBinaryAndCompareStdout ${TestExecutablesDirectory} "fsntfsinfo" ${TestProfile} "" ${TestInput}
