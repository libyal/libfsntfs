...
def main():
  ...
  if not pyfsntfs_test_single_open_close_file(options.source, "r"):
    return False

  if not pyfsntfs_test_single_open_close_file(None, "r"):
    return False

  if not pyfsntfs_test_single_open_close_file(options.source, "w"):
    return False

  if not pyfsntfs_test_multi_open_close_file(options.source, "r"):
    return False

  if not pyfsntfs_test_single_open_close_file_object(options.source, "r"):
    return False

  if not pyfsntfs_test_single_open_close_file_object_with_dereference(
      options.source, "r"):
    return False

  if not pyfsntfs_test_multi_open_close_file_object(
      options.source, "r"):
    return False
