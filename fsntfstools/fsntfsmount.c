...
#endif
{
	const char *description = \
		"Use fsntfsmount to mount a New Technology File System (NTFS) volume.";

	fsntfstools_option_t options[ ] = {
		{ 'h', NULL, "shows this help" },
		{ 'o', "offset", "specify the volume offset in bytes" },
		{ 'v', NULL, "verbose output to stderr, while fsntfsmount will remain running in the foreground" },
		{ 'V', NULL, "print version" },
#if defined( HAVE_LIBFUSE ) || defined( HAVE_LIBFUSE3 ) || defined( HAVE_LIBOSXFUSE )
		{ 'X', "extended_options", "extended options to pass to sub system" },
#endif
		{ 0, "volume", "a New Technology File System (NTFS) volume" },
	...
	int number_of_options                       = (int) ( sizeof( options ) / sizeof( fsntfstools_option_t ) );
	int verbose                                 = 0;
	...
	if( fsntfstools_getopt_get_options_string(
	     options,
	     number_of_options,
	     options_string,
	     32 ) != 1 )
	                   ...
	                   options_string ) ) != (system_integer_t) -1 )
	{
		switch( option )
		{
			...
			default:
				...
				fsntfstools_getopt_usage_fprint(
				 stdout,
				 program,
				 description,
				 options,
