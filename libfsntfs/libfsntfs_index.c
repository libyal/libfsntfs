     ...
     libcerror_error_t **error )
{
	...
	if( (size_t) index_node_header->index_values_offset > ( data_size - sizeof( fsntfs_index_root_header_t ) ) )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_RUNTIME,
	...
	index_values_offset = index_node_header->index_values_offset + (uint32_t) sizeof( fsntfs_index_root_header_t );

	if( (size_t) index_node_header->size > ( data_size - sizeof( fsntfs_index_root_header_t ) ) )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_RUNTIME,
	...
	index_node_size = index_node_header->size - (uint32_t) sizeof( fsntfs_index_node_header_t );

	index->flags = index_node_header->flags;
