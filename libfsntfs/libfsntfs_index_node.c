     ...
     libcerror_error_t **error )
{
	...
	static char *function                = "libfsntfs_index_node_read_data";
	...
	if( (size_t) index_node->header->index_values_offset > data_size )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_RUNTIME,
	...
	index_values_offset = index_node->header->index_values_offset;

	if( (size_t) index_node->header->size > data_size )
	{
		libcerror_error_set(
		 error,
		 LIBCERROR_ERROR_DOMAIN_RUNTIME,
	...
	index_node_size = index_node->header->size - (uint32_t) sizeof( fsntfs_index_node_header_t );
